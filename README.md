...
A **peer** is a single computer or "node" that acts as both client and a server at the same time. 
![[Pasted image 20260505113838.jpg]]
Unlike a traditional setup where a client talks to a big central computer, in a P2P network, every participant has equal status and responsibilities.
...
By maintaining a `list<CHID>` where each entry is linked to the previous one via a hash, the ledger ensures that once an identity is registered, in cannot be changed.

#### Availability

Because every node in the `p2pnetwork` maintains its own copy of the ledger, the system remains functional even if some nodes go offline.
...
- `send_to_nodes(data)`: Sends a dictionary to all connected peers.
  - Used to broadcast a `newBlock` after a successful registration.

- `send_to_node(node, data)`: Sends data to one specific peer.
...
`node_message(self, connected_node, data)`
This is the most important function for the project. It is triggered whenever any data arrives.

- **Parameters**:
  - `connected_node`: The node object that sent the message.
  - `data`: The dictionary containing the custom protocol (e.g., `action`, `childHash` ).
This is where the `if/elif` statements to handle **Registration**, **Querying**, or **Verification Responses**.
